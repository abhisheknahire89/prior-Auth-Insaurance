...
export const AdmissionCostStep: React.FC<AdmissionCostStepProps> = ({
    ...
    const [showRecalcNotification, setShowRecalcNotification] = useState(false);
    const [prevDiagnosisCode, setPrevDiagnosisCode] = useState<string | undefined>(undefined);
    ...
    useEffect(() => {
        ...
        if (dx) {
            ...
            const currentDiagnosisCode = dx.icd10Code;
            const diagnosisChanged = prevDiagnosisCode !== undefined && prevDiagnosisCode !== currentDiagnosisCode;

            // Trigger recalc if: first load, diagnosis changed, or cost was cleared
            const shouldRecalc = !admission.expectedDaysInRoom || hasEmptyCosts;

            if (shouldRecalc) {
                // Show notification if diagnosis was changed (not on first load)
                if (diagnosisChanged && admission.expectedDaysInRoom) {
                    setShowRecalcNotification(true);
                    setTimeout(() => setShowRecalcNotification(false), 3500);
                }

                setPrevDiagnosisCode(currentDiagnosisCode);
        ...
        clinical?.selectedDiagnosisIndex,
        clinical?.diagnoses?.[clinical?.selectedDiagnosisIndex ?? 0]?.icd10Code,
        clinical?.diagnoses?.[clinical?.selectedDiagnosisIndex ?? 0]?.diagnosis,
    ]);
    ...
    return (
        <div className="space-y-5 text-opd-text-primary">
            ...
            {showRecalcNotification && (
                <div className="bg-blue-50 border border-blue-300 rounded-lg p-3 text-sm text-blue-800 flex items-start gap-2 animate-slide-in">
                    <span className="text-base flex-shrink-0">🔄</span>
                    <span>
                        <strong>Cost estimate updated</strong> — Based on the selected diagnosis, admission timeline and room rent cap calculations have been recalculated.
                    </span>
                </div>
            )}
