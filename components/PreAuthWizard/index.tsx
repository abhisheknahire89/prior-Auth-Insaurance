...
import { getStepValidationErrors, ValidationError } from '../../utils/wizardValidation';
...
export const PreAuthWizard: React.FC<PreAuthWizardProps> = ({ 
    ...
    const [pendingStepAfterValidation, setPendingStepAfterValidation] = useState<number | null>(null);
    ...
    const handleNext = async () => {
        ...
        const errors = getStepValidationErrors(step, recordRef.current);
        if (errors.length > 0) {
            setValidationErrors(errors);
            setShowValidationModal(true);
            setPendingStepAfterValidation(step + 1);
            return;
        }
    ...
    const handleContinueWithValidationErrors = () => {
        setShowValidationModal(false);
        if (pendingStepAfterValidation) {
            setStep(pendingStepAfterValidation as any);
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-opd-text-primary/45 backdrop-blur-sm overflow-y-auto">
            <div className="wizard-modal-container bg-white border border-opd-border rounded-2xl w-full max-w-5xl my-8 mx-4 shadow-2xl overflow-hidden flex flex-col" style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <ValidationModal
                    ...
                    step={step}
                    record={record}
                    onContinue={handleContinueWithValidationErrors}
                    onFix={() => {
                        setShowValidationModal(false);
                        // User will manually fix and try again
                    }}
                    onCancel={() => {
                        setShowValidationModal(false);
                        setPendingStepAfterValidation(null);
