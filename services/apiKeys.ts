...
if (isBrowser) {
    ...
    const demoMode = (import.meta as any).env?.VITE_DEMO_MODE === 'true' || process.env.VITE_DEMO_MODE === 'true';

    if (!key && !demoMode) {
        console.warn("WARNING: VITE_GEMINI_API_KEY is missing from environment variables. Running in limited mode.");
    }
}
